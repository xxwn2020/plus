...
class SmsController extends Controller
{
     ...
     * @author Seven Du <shiweidu@outlook.com>
     */
    public function updateAlidayuOption(
    Configuration $store,
    Request $request,
    ResponseFactory $response
  ) {
    ...
    public function smsTemplate(Repository $config)
    {
        ...
        $data['alidayu_template_id']
      = $config->get('sms.channels.code.alidayu.template');
        $data['aliyun_template_id']
      = $config->get('sms.channels.code.aliyun.template');
        $data['yunpian_template_content']
      = $config->get('sms.channels.code.yunpian.content');
        $data['huyi_template_content']
      = $config->get('sms.channels.code.huyi.content');
    ...
    public function saveSmsConfig(Request $request, Configuration $store)
    {
        ...
        if (count($configures['allowed_gateways'])) {
            foreach (['gateways', 'allowed_gateways'] as $value) {
                $config->set(sprintf('sms.default.%s', $value),
          $configures['allowed_gateways']);
            }
        }
