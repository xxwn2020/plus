    ...
    <el-main>
      ...
      >
        ...
        <el-table-column width="60" prop="sex" :label="$t('admin.users.search.sex.root')">
          <template
            slot-scope="scope"
          >{{scope.row.sex === 0 ? $t('admin.users.search.sex.hide ') : (scope.row.sex === 1 ? $t('admin.users.search.sex.male ') : $t('admin.users.search.sex.female '))}}</template>
        </el-table-column>
        ...
        <el-table-column fixed="right" :label="$t('admin.operation')">
          <template slot-scope="scope">
            ...
            <template>
              ...
              >{{ $t('admin.users.recommend') }}</el-button>
              <el-button
                v-else
                size="mini"
                type="danger"
                @click="handleUnRecommend(scope.row)"
              >{{ $t('admin.users.unrecommend') }}</el-button>
            </template>
            <template v-if="scope.row.famous === null">
              ...
              >{{ $t('admin.users.followTa') }}</el-button>
              <el-button
                size="mini"
                type="primary"
                @click="handleFollowedFamous(scope.row, 2)"
              >{{ $t('admin.users.followEachOthers') }}</el-button>
            </template>
            <template v-else>
              <el-button
                size="mini"
              ...
              >{{ $t('admin.users.cancelFollow') }}</el-button>
            </template>
            <el-button
            ...
            >{{$t('admin.users.disable')}}</el-button>
            <el-button
              v-else
              size="mini"
              type="primary"
              @click="handleRestore(scope.row)"
            >{{$t('admin.users.restore')}}</el-button>
          </template>
        </el-table-column>
