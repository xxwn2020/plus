  ...
  <el-card shadow="never" class="add-certificated-page">
    ...
    <el-form :rules="rules" :model="certificated" ref="certificated" label-width>
      <el-form-item prop="username" :label="$t('admin.username')">
        <el-input :disabled="true" v-model="certificated.username"></el-input>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      fetchCertificated () {
        ...
        if (!getLoading) {
          this.$api.certifications.get(this.id).then(({ data }) => {
            const {
              certification_name: type,
              user_id,
              front,
              data: { name, number, phone, desc, org_name, org_address },
              back = null,
              user: { name: username },
              id
            } = data
            this.$set(this, 'frontUrl', File.url(front))
            back && this.$set(this, 'backUrl', File.url(back))
            this.$set(this, 'certificated', {
              id,
              user_id,
              name,
              number,
