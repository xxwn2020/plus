 ...
 *
 * @param blob
 * @param fileInfo 包含 name 和 mimeType
 * @param url dataUrl
 * @param callback 回调
 */
var upload = function(blob, fileInfo, url, callback) {
  ...
  reader.onload = function(e) {
    ...
    var params = {
      filename: md5(fileInfo.name) + '.' + fileInfo.name.split('.').splice(-1),
      hash: hash,
      size: blob.size,
...
var fileUpload = {
  ...
  isUploaded: function(image, f, callback) {
    ...
    reader.onload = function(e) {
      ...
      axios.get('/api/v2/files/uploaded/' + hash).then(function(response) {
  ...
  uploadFile: function(image, f, callback) {
    ...
    axios.post('/api/v2/files', formDatas).then(function(response) {
      if (response.data.id > 0) callback(image, f, response.data.id)
    }).catch(function(error) {
      ...
      then(function(response) {
        ...
        if (res.data != '') {
          ...
          if (_this.setting.paramtype == 0) {
            ...
            _this.params.offset = _this.setting.loadcount *
              _this.params.limit
          } else { // 热门动态
            _this.params.hot = res.after
          }
...
var checkNums = function(obj, len, show) {
  ...
  if (surplus < 0) {
    $(obj).parent().find('.' + show).text(surplus).css('color', 'red')
  } else {
    $(obj).parent().find('.' + show).text(surplus).css('color', '#59b6d7')
  }
}
...
var comment = {
  ...
  publish: function(obj) {
    ...
    _this.lockStatus = 1
    axios.post(comment.urls(_this.support.row_id, _this.support.type),
        ...
        then(function(res) {
          ...
          if (result.length) {
            ...
            result.forEach(function(user) {
              ...
              $('.ev-view-comment-follow-users').
                append(
                  '<li data-user-id="' + user.id + '" data-user-name="' +
...
var liked = {
  ...
  like: function(row_id, cate, type) {
    ...
    _this.lockStatus = 1
    axios.post(_this.res.link).then(function(response) {
      _this.num++
      _this.lockStatus = 0
  ...
  unlike: function(feed_id, page) {
    ...
    _this.lockStatus = 1
    axios.delete(_this.res.unlink).then(function(response) {
      _this.num--
      _this.lockStatus = 0
      _this.box.attr('rel', _this.num)
...
var pinneds = {
  ...
  show: function(url, type) {
    ...
    var html = '<div class="pinned_box">'
      ...
      TS.CURRENCY_UNIT + '余额为' +
      (TS.USER.currency ? TS.USER.currency.sum : 0) + '</div>'
...
var reported = {
  ...
  report: function() {
    ...
    ly.confirm(html, '', '', function() {
      ...
      if (_this.type === 'topic') {
        axios.put(_this.url, { message: reason }).then(function(res) {
          layer.closeAll()
          noticebox('举报成功', 1)
        }).catch(function(err) {
          showError(error.response.data)
  ...
  switch (type) {
    ...
    case 2: // QQ
      ...
      if (pic != '' && pic.indexOf('/api/v2/files') != -1) {
        axios.get(pic + '?json=1').then(function(response) {
...
var repostable = {
          ...
          if (result.length) {
            ...
            result.forEach(function(topic) {
              ...
              var nameMarked = topic.name.replace(regex,
                '<span style="color: #59b6d7;">$&<span>')
              $('.ev-view-repostable-topic-list').
                append('<li data-topic-id="' + topic.id +
                  '" data-topic-name="' + topic.name + '">' + nameMarked +
                  '</li>')
            })
          } else {
  ...
  post: function(type, id) {
    ...
    axios.post('/api/v2/feeds', data).then(function(response) {
...
if ('getContext' in document.createElement('canvas')) {
  ...
  HTMLImageElement.prototype.stop = function() {
    ...
    axios.get(that.dataset.original, {
      ...
      getFirstFrameOfGif(blob).then(function(b) {
    ...
    return this.each(function() {
      ...
      if (isIe6) {
        ...
        domThis.style.setExpression('right',
          'eval((document.documentElement).scrollRight + ' + o.x +
          ') + "px"')
